...
int isSuccessfulGroup(int nGroup)
{
   ...
   int nTriOne, nTriTwo, nTriThree;

   nTriOne = isSuccessfulTrio(nGroup, 1, 2, 3);
   nTriTwo = isSuccessfulTrio(nGroup, 4, 5, 6);
   nTriThree = isSuccessfulTrio(nGroup, 7, 8, 9);

   if (nTriOne == 1 && nTriTwo == 1 && nTriThree == 1){
      return 1;
   }
   else{
      return 0;
